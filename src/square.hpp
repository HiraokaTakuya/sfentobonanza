...
// FOREACH_BB マクロなどのなかで使用する。
// マクロの中では 2 つ以上のテンプレート引数の関数を使用出来無い為。
#define ISINFRONTOF(us, brank, wrank, target) isInFrontOf<us, brank, wrank>(target)
...
// todo: コメントアウトしている Bitboard を使用する方と、Direction を使用する方とどちらが速いか比較すること。
//       Bitboard の方が速い場合、Direction 型自体が必要なくなるかもしれない。
...
// 後手の位置を先手の位置へ変換
inline Square inverse(const Square sq) {
	return SquareNum - 1 - sq;
}
// 左右変換
inline File inverse(const File f) {
	return FileNum - 1 - f;
}
// 上下変換
inline Rank inverse(const Rank r) {
	return RankNum - 1 - r;
}
// Square の左右だけ変換
