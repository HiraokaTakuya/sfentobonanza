...
namespace {
	...
	// 非常に見難いが、持ち駒が多い局面ではかなり速度差があるので、仕方ない。
	// ちなみに、LS3600 氏は持ち駒の歩の有無×香の有無×桂の有無×銀金飛角の種類の数(0〜4種) = 2×2×2×5通り = 40通りでunroll
	template<Color US>
	MoveStack* generateDropMoves(MoveStack* moveStackList, const Position& pos, const Bitboard& target) {
		...
		if(hand.exists<HPawn>()) {
			...
			toBB &= ~rankMask<TRank9>();

			// 二歩の回避
			Bitboard pawnsBB = pos.bbOf(Pawn, US);
			while(pawnsBB.p(0)) {
				const Square pawnsSquare = pawnsBB.firstOneRightFromI9();
				toBB.set(0, toBB.p(0) & ~squareFileMask(pawnsSquare).p(0));
			}
			while(pawnsBB.p(1)) {
				const Square pawnsSquare = pawnsBB.firstOneLeftFromB9();
				toBB.set(1, toBB.p(1) & ~squareFileMask(pawnsSquare).p(1));
			}
		...
		if(hand.exceptPawnExists()) {
			...
			if(hand.exists<HKnight>()) {
				haveHand[haveHandNum++] = Knight;
			}
			const int noKnightIdx = haveHandNum; // 桂馬を除く駒でループするときのループの初期値
			if(hand.exists<HLance>()) {
				haveHand[haveHandNum++] = Lance;
			}
			const int noKnightLanceIdx = haveHandNum; // 桂馬, 香車を除く駒でループするときのループの初期値
			if(hand.exists<HSilver>()) {
				haveHand[haveHandNum++] = Silver;
			}
			if(hand.exists<HGold>()) {
				haveHand[haveHandNum++] = Gold;
			}
			if(hand.exists<HBishop>()) {
				haveHand[haveHandNum++] = Bishop;
			}
			if(hand.exists<HRook>()) {
				haveHand[haveHandNum++] = Rook;
			}
	...
	template<MoveType MT, Color US, bool ALL> struct GeneratePieceMoves<MT, Lance, US, ALL> {
		FORCE_INLINE MoveStack* operator () (MoveStack* moveStackList, const Position& pos, const Bitboard& target, const Square ksq) {
			...
			while(fromBB.p(0)) {
				const Square from = fromBB.firstOneRightFromI9();
				Bitboard toBB;
				toBB.set(0, pos.attacksFrom<Lance>(US, from).p(0) & target.p(0));
				while(toBB.p(0)) {
					const Square to = toBB.firstOneRightFromI9();
			...
			while(fromBB.p(1)) {
				const Square from = fromBB.firstOneLeftFromB9();
				Bitboard toBB;
				toBB.set(1, pos.attacksFrom<Lance>(US, from).p(1) & target.p(1));
				while(toBB.p(1)) {
					const Square to = toBB.firstOneLeftFromB9();
					const Rank toRank = makeRank(to);
					if(MT == Capture || MT == NonCapture || MT == NonEvasion || MT == Evasion || MT == CapturePlusPro) {
						if(isInFrontOf<US, Rank7, Rank3>(toRank)) {
							// 1, 2 段目は成りのみを生成する。
	...
	template<MoveType MT, Color US, bool ALL> struct GeneratePieceMoves<MT, Silver, US, ALL> {
		FORCE_INLINE MoveStack* operator () (MoveStack* moveStackList, const Position& pos, const Bitboard& target, const Square ksq) {
			...
			while(fromBB.isNot0()) {
				const Square from = fromBB.firstOneFromI9();
				Bitboard toBB = pos.attacksFrom<Silver>(US, from) & target;
				Bitboard toOn7BB = toBB & TRank789BB; // 3段目(Rank7) だけ
				// 前進する際には、成り + 不成
				if(toOn7BB.isNot0()) {
					toBB &= ~TRank789BB;
					do {
						const Square to = toOn7BB.firstOneFromI9();
						(*moveStackList++).move = makePromoteMove<MT>(Silver, from, to, pos);
						(*moveStackList++).move = makeNonPromoteMove<MT>(Silver, from, to, pos);
					} while(toOn7BB.isNot0());
				}
				// 後退する際には、不成
				while(toBB.isNot0()) {
					const Square to = toBB.firstOneFromI9();
	...
	FORCE_INLINE MoveStack* generateRecaptureMoves(MoveStack* moveStackList, const Position& pos, const Square to, const Color us) {
		Bitboard fromBB = pos.attacksTo(us, to);
		while(fromBB.isNot0()) {
			const Square from = fromBB.firstOneFromI9();
			const PieceType pt = pieceToPieceType(pos.piece(from));
			switch(pt) {
